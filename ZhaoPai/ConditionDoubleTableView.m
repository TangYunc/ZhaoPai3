...
@interface ConditionDoubleTableView ()
...
- (id)initWithFrame:(CGRect)frame andLeftItems:(NSArray *)leftItems andRightItems:(NSArray *)RightItems{
    ...
    self = [super init];
    if (self) {
        self.view.frame = CGRectMake(0, 40, frame.size.width, frame.size.height - 40);
        ...
        UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, frame.size.height / 2, frame.size.width, frame.size.height / 2 - 40)];
        //为视图添加手势，点击消失菜单
...
- (void)initSecondTableViewWithFrame:(CGRect)frame {
    _secondTableView = [[UITableView alloc] initWithFrame:CGRectMake(frame.size.width / 2, 0, frame.size.width, _firstTableView.height) style:UITableViewStylePlain];
...
- (void)showSingleOrDouble {
    if (_leftArray.count <= 0) {
        ...
        _secondTableView.frame = CGRectMake( 0, 0, kScreenWidth, kScreenHeight / 2);
    } else {
        [_firstTableView setHidden:NO];
        _secondTableView.frame = CGRectMake(kScreenWidth / 2, 0, kScreenWidth, kScreenHeight / 2);
    }
}
